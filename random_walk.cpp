...
const int n_particle = 10;
Vec2 particle[n_particle];
RGBA color[n_particle];
Image trail;
...
void update(float dt) {
    if (every_seconds(0.01, dt)) {
        image_multiply_scalar(&trail, 0.95);
        for (int i = 0; i < n_particle; ++i) {
            bool horizontal = randf() >= 0.5;
            bool direction = randf() >= 0.5;
            Vec2 position = particle[i];
            if (horizontal) {
                position.x += direction ? -1 : 1;
                color[i].r = clampf(color[i].r + (direction ? 0.1 : -0.1), 0, 1);
            } else {
                position.y += direction ? -1 : 1;
                color[i].b = clampf(color[i].b + (direction ? 0.1 : -0.1), 0, 1);
            }

            particle[i] = position;
            image_set(&trail, particle[i].x, particle[i].y, color[i]);
        }
    }
}
